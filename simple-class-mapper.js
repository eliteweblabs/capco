...
/**
 * Simple Class Mapper - Direct approach to handle obfuscated classes
 */
    ...
    if (line.includes("@media") && line.includes("min-width")) {
      ...
      for (let j = i + 1; j < i + 10 && j < lines.length; j++) {
        ...
        if (nextLine.includes(`.${className}`)) {
          ...
          // Extract properties
          const properties = extractProperties(nextLine);
...
Usage: node simple-class-mapper.js [obfuscated-class-name]

Examples:
  node simple-class-mapper.js fE3pmEmw8F30VPtAqcha
  node simple-class-mapper.js Q_jg_EPdNf9eDMn1mLI2
`);
  process.exit(1);
}

console.log(`🔍 Simple analysis for class: ${className}`);
