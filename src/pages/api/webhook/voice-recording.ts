...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    console.log("🎙️ [VOICE-RECORDING] Recording webhook received:", body);
    ...
    console.log("🎙️ [VOICE-RECORDING] Recording details:", {
      recordingUrl,
      callId,
      conversationId,
      duration,
    ...
    if (recordingUrl) {
      ...
      try {
        ...
        console.log("🎙️ [VOICE-RECORDING] Forwarding recording to n8n:", n8nWebhookUrl);
        ...
        if (response.ok) {
          console.log("✅ [VOICE-RECORDING] Successfully forwarded to n8n");
        } else {
          console.error("❌ [VOICE-RECORDING] Failed to forward to n8n:", response.status);
        }
      } catch (error) {
        console.error("❌ [VOICE-RECORDING] Error forwarding to n8n:", error);
      }
    }
