 ...
 * and labor USD rolled up from each entry's hourlyRateSnapshot (rate at check-in).
...
function generateDummyMetrics() {
  ...
  const hoursPerProject = projects.map((p, i) => {
    const totalHours = [24.5, 18.2, 12.8, 8.1][i] ?? 10;
    return {
      projectId: p.id,
      project: p.title,
      totalHours,
      totalLaborUsd: Math.round(totalHours * hourly * 100) / 100,
  ...
  users.forEach((u, ui) => {
    projects.forEach((p, pi) => {
      ...
      const totalHours = Math.round(h * 10) / 10;
  ...
  const totalLaborUsd = hoursPerProject.reduce((s, p) => s + p.totalLaborUsd, 0);
  const totalEntries = hoursPerProject.reduce((s, p) => s + p.entryCount, 0);

  const monthlyTrend = [];
  for (let i = 5; i >= 0; i--) {
    const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
    const mh = Math.round((60 + Math.random() * 40) * 10) / 10;
    monthlyTrend.push({
      month: d.toISOString().slice(0, 7),
      label: d.toLocaleDateString("en-US", { month: "short", year: "numeric" }),
      totalHours: mh,
      totalLaborUsd: Math.round(mh * hourly * 100) / 100,
  ...
  return {
    totalHours,
    totalLaborUsd: Math.round(totalLaborUsd * 100) / 100,
    totalEntries,
    hoursPerProject,
...
export const GET: APIRoute = async ({ request, cookies }): Promise<Response> => {
  try {
    ...
    const projectHours = new Map<number, { hours: number; count: number; laborUsd: number }>();
    const userProjectAgg = new Map<string, { hours: number; laborUsd: number }>();
    const monthMap = new Map<string, { hours: number; count: number; laborUsd: number }>();

    for (const e of list) {
      const start = new Date(e.startedAt).getTime();
      const end = new Date(e.endedAt).getTime();
      const hours = (end - start) / (1000 * 60 * 60);
      const rateRaw = (e as { hourlyRateSnapshot?: unknown }).hourlyRateSnapshot;
      const rate =
        rateRaw != null && rateRaw !== ""
          ...
          : null;
      const lineLabor =
        rate != null && Number.isFinite(rate) && rate >= 0 ? hours * rate : 0;

      const pid = e.projectId ?? 0;

      if (!projectHours.has(pid)) projectHours.set(pid, { hours: 0, count: 0, laborUsd: 0 });
      const ph = projectHours.get(pid)!;
      ph.hours += hours;
      ph.count += 1;
      ph.laborUsd += lineLabor;

      const upKey = `${e.userId}|${pid}`;
      const prev = userProjectAgg.get(upKey) ?? { hours: 0, laborUsd: 0 };
      prev.hours += hours;
      prev.laborUsd += lineLabor;
      userProjectAgg.set(upKey, prev);

      const d = new Date(e.startedAt);
      const month = d.toISOString().slice(0, 7);
      if (!monthMap.has(month)) monthMap.set(month, { hours: 0, count: 0, laborUsd: 0 });
      const m = monthMap.get(month)!;
      m.hours += hours;
      m.count += 1;
      m.laborUsd += lineLabor;
    }

    const hoursPerProject = Array.from(projectHours.entries()).map(
      ([projectId, { hours, count, laborUsd }]) => ({
        projectId,
        project: projectId
          ? (projectMap.get(projectId) ?? `#${projectId}`)
          : "General / No Project",
        totalHours: Math.round(hours * 100) / 100,
        totalLaborUsd: Math.round(laborUsd * 100) / 100,
        entryCount: count,
      })
    );
    ...
    for (const [key, agg] of userProjectAgg) {
      ...
      hoursPerUserPerProject.push({
        ...
        totalLaborUsd: Math.round(agg.laborUsd * 100) / 100,
      });
    }

    const totalHours = hoursPerProject.reduce((s, p) => s + p.totalHours, 0);
    const totalLaborUsd = hoursPerProject.reduce((s, p) => s + (p.totalLaborUsd ?? 0), 0);
    const totalEntries = list.length;

    const monthlyTrend = Array.from(monthMap.entries())
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([month, { hours, count, laborUsd }]) => ({
        month,
