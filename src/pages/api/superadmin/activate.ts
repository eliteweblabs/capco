...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const secret = import.meta.env?.SUPERADMIN_SECRET ?? process.env?.SUPERADMIN_SECRET;
    if (!secret) {
    ...
    if (passphrase !== secret) {
      return new Response(JSON.stringify({ success: false, error: "Invalid passphrase" }), {
        status: 403,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (!payload) {
      return new Response(JSON.stringify({ success: false, error: "Could not create token" }), {
        status: 500,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({ success: true, message: "SuperAdmin activated" }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (e) {
