...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!emailData.from || !emailData.subject) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Missing required email data" }), {
        status: 400,
    ...
    if (!user) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Failed to find or create user" }), {
        status: 500,
        headers: { "Content-Type": "application/json" },
      });
    }
...
async function createInitialDiscussion(projectId: number, userId: string, emailData: EmailWebhookData) {
  try {
