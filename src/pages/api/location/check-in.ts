...
export const POST: APIRoute = async ({ request, cookies }): Promise<Response> => {
  try {
    ...
    const message = isCheckIn
      ? `${userName} checked in at ${timeStr}. Coordinates: ${coords}`
      : `${userName} checked out at ${timeStr}. Last location: ${coords}`;
    ...
    if (usersError || !adminUsers?.length) {
      ...
      return new Response(
        JSON.stringify({ success: true, count: 0, message: "No admins to notify" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }
