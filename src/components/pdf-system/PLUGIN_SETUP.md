...
cp -r src/components/pdf-system ./src/components/pdf-system
```
...
```bash
cp src/components/pdf-system/pdf-system.env.example .env.local
```
...
| `PDF_TEMPLATES_DIR` | `src/components/pdf-system/templates` | HTML templates directory |
...
```bash
# Component files
cp -r src/components/pdf-system ./
...
# Copy component
cp -r src/components/pdf-system $TARGET_PROJECT/src/components/
