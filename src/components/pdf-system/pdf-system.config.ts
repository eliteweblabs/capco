...
export const defaultConfig: PDFSystemConfig = {
  ...
  templatesDirectory: process.env.PDF_TEMPLATES_DIR || "src/components/pdf-system/templates",
  ...
  pdf: {
    ...
    defaultOrientation: (process.env.PDF_DEFAULT_ORIENTATION as "portrait" | "landscape") || "portrait",
    defaultMargins: {
...
export function loadConfig(customConfig?: Partial<PDFSystemConfig>): PDFSystemConfig {
  const envConfig: Partial<PDFSystemConfig> = {
    saveHtmlTemplates: process.env.SAVE_HTML_TEMPLATES === "true" || false,
    templatesDirectory: process.env.PDF_TEMPLATES_DIR,
    supabase: {
    ...
    pdf: {
      ...
      defaultOrientation: process.env.PDF_DEFAULT_ORIENTATION as "portrait" | "landscape",
      defaultMargins: {
