 ...
 *
 * Overscroll ramp: For #reveal-scroll layout, uses scrollEl.clientHeight so scale effects
 * (header cover, footer translate) respond quickly to overscroll. innerHeight makes ramp too slow.
 */
(window as any).getOverscrollPercent = function (): number {
  ...
  const rampH = useEl && scrollEl ? scrollEl.clientHeight || 1 : window.innerHeight || 1;

  if (useEl && scrollEl) {
    const maxScroll = Math.max(0, scrollEl.scrollHeight - scrollEl.clientHeight);
    if (maxScroll <= 0) return 100;
    const top = scrollEl.scrollTop;
    if (top > maxScroll) return 100 + ((top - maxScroll) / rampH) * 100;
    if (top < 0) return (top / rampH) * 100;
...
(window as any).setupOverscrollStart = function (
  ...
  function onWheel(ev: WheelEvent) {
    ...
    if (atBottom && scrollingDown) {
      ...
      const raw = (wheelAccum / rampH) * 100;
