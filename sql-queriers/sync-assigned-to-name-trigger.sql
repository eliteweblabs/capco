...
BEGIN
  ...
  IF NEW.assigned_to_id IS DISTINCT FROM OLD.assigned_to_id THEN
    IF NEW.assigned_to_id IS NOT NULL THEN
      SELECT company_name INTO NEW.assigned_to_name
      FROM profiles 
      WHERE id = NEW.assigned_to_id;
    ELSE
      -- If assigned_to_id is set to NULL, clear assigned_to_name
...
RETURNS TRIGGER AS $$
BEGIN
  -- Set assigned_to_name from the assigned user's profile when creating a new project
  IF NEW.assigned_to_id IS NOT NULL AND NEW.assigned_to_name IS NULL THEN
    SELECT company_name INTO NEW.assigned_to_name
    FROM profiles 
    WHERE id = NEW.assigned_to_id;
  END IF;
