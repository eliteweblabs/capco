-- Replace CAPCo Fire with {{RAILWAY_PROJECT_NAME}} placeholder in project statuses
...
-- Step 1: Replace 'CAPCo Fire' with {{RAILWAY_PROJECT_NAME}} in all text columns
DO $$
DECLARE
    col_name text;
    update_sql text;
BEGIN
    ...
    LOOP
        ...
        update_sql := format('UPDATE project_statuses SET %I = REPLACE(%I, %L, %L) WHERE %I LIKE %L', 
                             col_name, col_name, 'CAPCo Fire', '{{RAILWAY_PROJECT_NAME}}', col_name, '%CAPCo Fire%');
...
-- Step 2: Replace 'CAPCo' with {{RAILWAY_PROJECT_NAME}} in all text columns
DO $$
DECLARE
    col_name text;
    update_sql text;
BEGIN
    ...
    LOOP
        -- Build dynamic UPDATE statement
        update_sql := format('UPDATE project_statuses SET %I = REPLACE(%I, %L, %L) WHERE %I LIKE %L', 
                             col_name, col_name, 'CAPCo', '{{RAILWAY_PROJECT_NAME}}', col_name, '%CAPCo%');
...
-- Show results - find all rows that now contain {{RAILWAY_PROJECT_NAME}}
...
WHERE EXISTS (
    ...
    WHERE table_name = 'project_statuses' 
      ...
      AND (
        ...
        WHERE col_value LIKE '%{{RAILWAY_PROJECT_NAME}}%'
      ) > 0
);
