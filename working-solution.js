...
/**
 * Working Solution - Correctly handles obfuscated classes with responsive behavior
 */
    ...
    if (line.includes("@media") && line.includes("min-width")) {
      ...
      for (let j = i + 1; j < i + 10 && j < lines.length; j++) {
        ...
        if (nextLine.includes(`.${className}`)) {
          ...
          // Extract properties - fix the extraction
...
Usage: node working-solution.js [obfuscated-class-name]

Examples:
  node working-solution.js fE3pmEmw8F30VPtAqcha
  node working-solution.js Q_jg_EPdNf9eDMn1mLI2
`);
  process.exit(1);
}
